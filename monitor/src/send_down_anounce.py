...
def create_gitlab_issue(title, description, labels=None):
    ...
    try:
        response = requests.post(
            url, headers=headers, data=json.dumps(data), timeout=20
        )
        response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)
        issue_data = response.json()
        print(
            f"Issue created successfully! ID: {issue_data['iid']}, URL: {issue_data['web_url']}"
        )
...
def generate_issue(service_name, host_address):
    """geneates the issue message"""
    issue_title = f"Service Down: {service_name}"
    issue_description = (
        f"Service Down: {service_name},"
        f" Service host address on cluster is: {host_address}"
    )
